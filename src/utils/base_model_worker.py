...
class BaseModelWorker:
    ...
    def make_conv_template(self, conv_template: str = None, model_path: str = None) -> Conversation:
        """
        can be overrided to customize the conversation template for different model workers.
        """
    ...
    def register_to_controller(self):
        ...
        # host_name = os.getenv("HOSTNAME", "")  # ali: dlc7aay2vy9kru0v-master-0
        # host_ip = "0.0.0.0"
        # if 'SH-IDC1' in host_name:
        #     host_ip = host_name.replace('SH-IDC1-', '').replace('-', '.')
        # else:
        #     import psutil
        #     import socket
        #
        ...
        #     if "net0" in addrs:
        #         for addr in addrs["net0"]:
        #             if addr.family == socket.AF_INET:  # IPv4
        #                 host_ip = addr.address  # ali: 10.1.11.226
        # url = url.replace('0.0.0.0', host_ip)
        # self.worker_addr = self.worker_addr.replace('0.0.0.0', host_ip)
    ...
    def get_queue_length(self):
        if self.semaphore is None:
            ...
            sempahore_value = self.semaphore._value if self.semaphore._value is not None else self.limit_worker_concurrency
...
@app.post("/worker_generative_reward_inference")
async def generative_reward_inference(request: Request):
    params = await request.json()
    await acquire_worker_semaphore()
    output = await asyncio.to_thread(worker.generative_reward_inference_gate, params)
    release_worker_semaphore()
    return JSONResponse(output)
